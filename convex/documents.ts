...
export const addFormula = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    const formulas = existingDocument.formulas || [];
    const newFormula = {
...
export const updateFormula = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const updatedFormulas = formulas.map((f) =>
      f.id === args.formulaId ? { ...f, formula: args.formula } : f
    );
...
export const removeFormula = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const formulas = existingDocument.formulas || [];
    const updatedFormulas = formulas.filter((f) => f.id !== args.formulaId);
